...
SiteTreat <- read.csv("RR_SiteTreatment.csv")##list sites and corresponding treatments

#Plant C-Stock model:=========
Plant_CarbonStock <- select (aa, Site,Treatment2, Total_Aboveground_Biomass_kg_100m2) %>%
  mutate(Ecosystem = Treatment2,
         Plant_Biomass = Total_Aboveground_Biomass_kg_100m2 / 10 ) %>% #1 kg/100m2 = 0.1 tonnes/ha
  na.omit() %>%
...
#Run Plant Stock model:
plant_model <- lm(Plant_Biomass ~ Ecosystem, data = Plant_CarbonStock)
summary(plant_model )
tab_model(plant_model)

#Run Plant Stock ANOVA:
plant_aov <- aov(Plant_Biomass ~ Ecosystem, data = Plant_CarbonStock)
summary(plant_aov )
tab_model(plant_aov )
sjp.aov1 (Plant_CarbonStock$Plant_Biomass,Plant_CarbonStock$Ecosystem,
          show.summary=T, show.p = T)

#Soil Stock model till 1m deep:====
...
soil_carbon$Ecosystem <- soil_carbon$Treatment2
